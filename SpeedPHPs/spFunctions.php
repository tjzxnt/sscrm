...
function spError($msg, $output = TRUE, $stop = TRUE){
	if(TRUE != SP_DEBUG){
		error_log($msg);
		if(TRUE == $stop){
			header( "Location:http://" . $_SERVER['SERVER_NAME'] . "/" . $GLOBALS['G_SP']['nofound']);
			exit;
		}
	}
 ...
 */
function spUrl($controller = null, $action = null, $args = null, $anchor = null, $no_sphtml = FALSE, $oralcode = 0)
{
	if(!$oralcode && TRUE == $GLOBALS['G_SP']['html']["enabled_zxadv"] || TRUE == $GLOBALS['G_SP']['html']["enabled"] && TRUE != $no_sphtml){
		// 当开启HTML生成时，将查找HTML列表获取静态文件名称。
		return spUrl_htmladv($controller, $action, $args, $anchor, $no_sphtml);
	}
	...
	// 使用挂靠点
	if( $launch = spLaunch("function_url", array('controller'=>$controller, 'action'=>$action, 'args'=>$args, 'anchor'=>$anchor, 'no_sphtml'=>$no_sphtml), TRUE ))return trim($launch);
...
function spUrl_htmladv($controller = null, $action = null, $args = null, $anchor = null, $no_sphtml = FALSE)
{
	if(TRUE == $GLOBALS['G_SP']['html']["enabled_zxadv"] || TRUE == $GLOBALS['G_SP']['html']["enabled"] && TRUE != $no_sphtml){
		// 当开启HTML生成时，将查找HTML列表获取静态文件名称。
		$realhtml = spHtml::getUrl_htmladv($controller, $action, $args, $anchor, null);
 ...
 * spInject  执行注入程序
 * 
 * @param entryName   注入点名称
 * @param args        注入参数
 * @param return      是否存在返回数据，如需要返回，则该注入点仅能有一个注入操作
 */
function spInject($entryName, $args = null, $returns = false ){
	if( isset($GLOBALS['G_SP']['inject'][$entryName]) && is_array($GLOBALS['G_SP']['inject'][$entryName]) ){
		foreach( $GLOBALS['G_SP']['inject'][$entryName] as $entry ){
			if( is_array($entry) ){
				$reval = spClass($entry[0])->{$entry[1]}($args);
			}
			else{
				$reval = call_user_func_array($entry, $args);
			}
			if($returns) {
				return $reval;
			}
