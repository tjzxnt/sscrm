...
class spModel {
	 ...
	 */
	public function find($conditions = null, $sort = null, $fields = null, $group = null)
	{
		if( $record = $this->findAll($conditions, $sort, $fields, $group = null, 1) ){
	 ...
	 */
	public function findByPk($pkvalue, $fields = null){
		if( $record = $this->findAll(array($this->pk => $pkvalue), null, $fields, null, 1) ){
			return array_pop($record);
		}else{
			return false;
		}
	 ...
	 * @param sort    排序，等同于“ORDER BY ”
	 * @param fields    返回的字段范围，默认为返回全部字段的值
	 * @param group   分组，等同于“GROUP BY ”	 
	 * @param limit    返回的结果数量限制，等同于“LIMIT ”，如$limit = " 3, 5"，即是从第3条记录（从0开始计算）开始获取，共获取5条记录
	 *                 如果limit值只有一个数字，则是指代从0条记录开始。
	 */
	public function findAll($conditions = null, $sort = null, $fields = null, $group = null, $limit = null)
	{
		return $this->_db->getAll($this->tbl_name, $conditions, $sort, $fields, $group, $limit);
	}
	 ...
	 *
	 * @param conditions   查找条件，数组array("字段名"=>"查找值")或字符串
	 * @param field        计数字段，默认为数据表主键字段
	 * @param clear	       清除条件
	 */
	public function findCount($conditions = null, $field = null, $clear = true){
		if(null == $field) {
			$field = $this->pk;
		}
		return $this->_db->findCount($this->tbl_name, $conditions, $field, $clear);
	}
	 ...
	 * @param  $join_cond  关联条件（若没有关联条件，则自动查找模型在定义的连接关系（$join数据））
	 *
	 * 两种Join表方法
	 * 方法1：第二个参数直接写连接条件
	 * 优点： 比较灵活，可以自行定义表的别名，多表连接时支持 A join B ， B join C的情况
	 * 缺点： 稍显复杂
	 * //使用方法
	 * $payments = JLoadClass('payments');
	 * $result = $payments->join('payment_types', 'payments.payment_type_id=payment_types.id')
	 *					  ->join('website_members', 'payments.member_id=website_members.id')
	 *					  ->findAll(null, null, 'payments.*,payment_types.name as payment_type, website_members.username');
	 *
	 * 方法2：第二个参数为空，连接关系在Model中定义(join数组)
	 * 优点： 比较简洁
	 * 缺点： 多表连接时 只支持A join B， A join C的情况
	 * //模型中关系定义
	 * var $join = array(
	 *		'payment_types' => array(
	 *			'mapkey' => 'id',
	 *			'fkey' => 'payment_type_id'
	 *		),
	 ...
	 * //使用方法
	 * $payments = JLoadClass('payments');
	 * $result = $payments->join('payment_types')
	 *					  ->join('website_members')
	 *					  ->findAll(null, null, 'payments.*,payment_types.name as payment_type, website_members.username');
	 */
	public function join($tbl_name, $join_cond = null, $type = 'inner') {
		if(null == $join_cond) {
			if(null != $this->join && isset($this->join[$tbl_name]) && is_array($this->join[$tbl_name]) ) {
				$join_cond = $this->tbl_name . '.' . $this->join[$tbl_name]['fkey'] . '=' .  $tbl_name . '.' . $this->join[$tbl_name]['mapkey'];
			}
			else {
				return $this;
			}
		}
		$this->_db->joinTable($tbl_name, $join_cond, $type);
		return $this;
	}
	 ...
	 */
	public function where($fields, $value = null, $relation = 'AND', $relation2 = 'AND') {
		$this->_db->addWhere($fields, $value, $relation, $relation2);
		return $this;
	}
	...
	public function create($row){
		...
		return $this->_db->create($this->tbl_name, $row);
	}
	...
	{
		return $this->_db->delete($this->tbl_name, $conditions);
	}
	...
	public function update($conditions, $row){
		return $this->_db->update($this->tbl_name, $conditions, $row);
	}
	...
	{
		return $this->update($conditions, array($field=>$optval));
	}
	...
	{	
		if($name == 'validator') {
			$this->validator = $this->$args[0] ? $this->$args[0] : $this->validator;
		}
		else if(in_array($name, $GLOBALS['G_SP']["auto_load_model"])){
			return spClass($name)->__input($this, $args);
		}
		else if($name == 'debug') {
			$this->_db->debug = $args[0];
		}
	 ...
	 * 获取数据校验规则JS对象
	 */
	public function getValidatorJS($validator = null) {
		$validator = $validator? $validator : $this->validator;
		if(null != $validator) {
			if($GLOBALS['G_SP']['view']['client_validator'] == 'Validation') {
				//调整规则以适应JQuery Validation插件
				foreach($validator['rules'] as $index=>$rules) {
					foreach($rules as $rule=>$value) {
						if($rule == 'equalTo') {
							$validator['rules'][$index][$rule] = '#' . $value;
						}
					}
				}
			}
...
class spPager {
	...
	private function runpager($func_name, $func_args){
		...
		@list($conditions, $sort, $fields, $group ) = $func_args;
		if('findSql'==$func_name){
			$total_count = array_pop( array_pop( $this->model_obj->findSql("SELECT COUNT(distinct {$this->model_obj->pk}) as sp_counter FROM ($conditions) sp_tmp_table_pager1") ) );
		}else{
			//$total_count = $this->model_obj->findCount($conditions, null, null, false);
			$temp_db = clone $this->model_obj->_db;
			$total_count = 0;
			if($total_rs = $this->model_obj->findAll($conditions, null, null, $group)) $total_count = count($total_rs);
			$this->model_obj->_db = $temp_db;
		}
		...
		if('findSql'==$func_name){
			...
			return $this->model_obj->findAll($conditions, $sort, $fields, $group, $limit);
		}
	}
}
...
/**
 * spValidator
 * 数据验证程序(校验规则与JQuery Validator一致)
 */
class spValidator {
	 ...
	 */
    public function __input(& $obj, $args){
		$this->validator = (null != $obj->validator) ? $obj->validator : array();
		if(isset($args[1]) && is_array($args[1])){
			//将参数中的规则与预定义的规则合并
			$this->validator["rules"] = $this->validator["rules"] + $args[1]["rules"];
			$this->validator["messages"] = isset($args[1]["messages"]) ? ( $this->validator["messages"] + $args[1]["messages"] ) : $this->validator["messages"];
		}
		if(is_array($obj->addrules) && !empty($obj->addrules) ){foreach($obj->addrules as $addrule => $addveri)$this->addrules($addrule, $addveri);}
		if(empty($this->validator["rules"]))spError("无对应的验证规则！");
	...
	private function checkrules($values){ 
		...
		foreach( $this->validator["rules"] as $rkey => $rval ){
			...
			foreach( $rval as $rule => $rightval ){
				...
				$this->messages[$rkey][] = (isset($this->validator["messages"][$rkey][$rule])) ? $this->validator["messages"][$rkey][$rule] : "{$rule}";
			}
		}
	 ...
	 * @param right    正确值
	 */
	private function required($val, $right){return $right === ( isset($val) && strlen($val) > 0 && "" != $val );}
	private function notnull($value, $right){return $this->required($value, $right);}
	/** 
	 * 内置验证器，检查字符串是否小于指定长度
	 * @param val    待验证字符串
	 * @param right    正确值
	 */
	private function minlength($val, $right){if($val == '') return 1; else return $this->cn_strlen($val) >= $right;}
	/** 
	 * 内置验证器，检查字符串是否大于指定长度
	 * @param val    待验证字符串
	 * @param right    正确值
	 */
	private function maxlength($val, $right){if($val == '') return 1; else return $this->cn_strlen($val) <= $right;}
	/** 
	...
	/** 
	 * 内置验证器，检查字符串是否仅包含英文、数字和下划线
	 * @param value    待验证字符串
	 * @param right    正确值
	 */
	private function alphanumeric($value, $right){return $right == ( preg_match('/^\w+$/', $value) != 0 );}
	 ...
	 * @param right    正确值
	 */
	public function Z_integer($val, $right){
		return $right == ( preg_match('/^\d+$/', $val) != 0 );
	}
	/** 
