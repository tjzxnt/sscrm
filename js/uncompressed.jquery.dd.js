...
;(function($) {
   ...
   var dd = function(element, options)
   {
	...
	var setTitleImageSprite = function() {
		if(options.useSprite!=false) {
			...
			if(sClassName.length>0) {
				//alert(sClassName);	
				var sStyle1 = getImageFromClassName("."+sClassName); //background Position
				//alert(sStyle1);
				var sStyle2 = getImageFromClassName("."+options.useSprite); //main
				//alert(sStyle2.backgroundImage + sStyle1.backgroundPosition);
				if(sStyle1 != null && sStyle2 != null) {
					var bgimage = sStyle2.backgroundImage.replace(/"/gi, "");
					bgimage = bgimage.replace(/..\//,"");
				}
				$("#"+titletextid).find("."+styles.ddTitleText).css('background-image', bgimage);
				if(sStyle1.backgroundPosition!=undefined){
				$("#"+titletextid).find("."+styles.ddTitleText).css('background-position', sStyle1.backgroundPosition);
				};
				$("#"+titletextid).find("."+styles.ddTitleText).css('background-repeat', 'no-repeat');
				if(sStyle2.paddingLeft!=undefined){
				$("#"+titletextid).find("."+styles.ddTitleText).css('padding-left', sStyle2.paddingLeft);
				};
	...
	var getImageFromClassName = function(selectorText) {
		if (document.styleSheets) {
			for (var i = document.styleSheets.length - 1; i >= 0; i--) {
				...
				if (rules) {
					var fullText2 = ('.'+options.mainCSS+' .ddChild a'+selectorText).toLowerCase(); //works in ie
					var fullText = ('.'+options.mainCSS+' .ddChild a'+selectorText+', .'+options.mainCSS+' .ddChild a'+selectorText+':visited').toLowerCase();
					for (var j = rules.length - 1; j >= 0; j--) {
						//console.debug(rules[j].selectorText);
					//	alert(rules[j].selectorText);
						if ((rules[j].selectorText).toLowerCase() == fullText || (rules[j].selectorText).toLowerCase() == fullText2) {
							var myStyle = new Object();
							for(var s in rules[j].style) {
								if(typeof(rules[j].style[s]) == "string" && rules[j].style[s].length>0) {
									//alert(s + " : " +rules[j].style[s])
									//console.debug(s +" : "+rules[j].style[s]);
									myStyle[s] = rules[j].style[s];
								};
							};
