<?php
class clientoverseas extends spController {
	...
	public function myclientlist(){
		...
		if($postdate['searchkey'] != '')
			$condition .= " and (crm_client.realname like '%{$postdate['searchkey']}%' or crm_client.telphone like '%{$postdate['searchkey']}%' or crm_client.cred_license like '%{$postdate['searchkey']}%')";
		if($postdate['process_id'].'a' !== 'a'){
			$condition .= " and crm_client.process_id = ".$postdate['process_id'];
			$this->process_id = $postdate['process_id'];
		}
		$this->client_rs = $obj_client->join("crm_user as sales_user", "sales_user.id = crm_client.user_sales_id")->join("crm_credential")->join("crm_country")->join("crm_client_process")->spPager($page, 20)->findAll($condition, 'crm_client.to_overseas_time desc', "crm_client.*, crm_credential.cname, crm_client_process.pname, sales_user.realname as realname_sales, crm_country.country as exp_country");
		$this->process_rs = $obj_process->getlist();
		$this->pager = $obj_client->spPager()->getPager();
		$this->searchkey = $postdate['searchkey'];
		$this->url = spUrl('clientoverseas', 'myclientlist', array("searchkey"=>$this->searchkey, "process_id"=>$this->process_id));
	}
	...
	public function viewmyoverseasclient(){
		try {
			...
			switch ($client_rs["sourcetype"]){
				case "1":
					exit();
					$this->display("clientoverseas/viewclient_channel.html");
				break;
				case "2":
				...
				case "3":
					...
					$this->display("clientoverseas/viewclient_unorigin.html");
				break;
			}
		}catch(Exception $e){
	...
	public function clientrecordlist(){
		try {
			...
			$postdate = $this->spArgs();
			$page = intval(max($postdate['page'], 1));
			$record_rs = $obj_record->join("crm_user")->join("crm_client")->spPager($page, 20)->findAll(array("crm_client_record.client_id"=>$client_id, "crm_client_record.rtype_id"=>1, "crm_client.user_overseas_id"=>$_SESSION["sscrm_user"]["id"]), "crm_client_record.createtime asc", "crm_client_record.*, crm_user.realname as realname_create");
			$this->record_rs = $record_rs;
			$this->pager = $obj_record->spPager()->getPager();
			$this->client_id = $client_id;
			$this->url = spUrl('clientoverseas', 'clientrecordlist', array("client_id"=>$client_id));
	...
	public function clientpayview(){
		try {
			...
			*/
			$this->display("clientsales/clientpayview.html");
		}catch(Exception $e){
			$this->redirect(spUrl("clientoverseas", "myclientlist"), $e->getMessage());
		}
	}
	...
	public function clientorderfundlist(){
		try {
			...
			$this->client_id = $client_id;
			$this->url = spUrl('clientoverseas', 'clientorderfundlist', array("client_id"=>$client_id));
			$this->clearurl = spUrl('clientoverseas', 'clientorderfundclear');
			$this->createurl = spUrl("clientoverseas", "createorderfund", array("client_id"=>$client_id));
			$this->backurl = spUrl("clientoverseas", "myclientlist");
	...
	public function clientorderfundclear(){
		try {
			...
			$fund_rs = $obj_fund->find(array("crm_client_order_fund.client_id"=>$client_id), null, "max(arrivaltime) as fullpay_arrivaltime");
			...
			spClass('user_log')->save_log(3, "将客户 ".$this->client_rs['realname']." [id:$client_id] 的订单 [" . $this->client_rs['bargain'] . "] 的合同款设置为可返佣", array("client_id"=>$client_id));
			echo json_encode(array("url"=>spUrl("clientoverseas", "clientorderfundlist", array("client_id"=>$client_id)), "msg"=>"已将该订单的房款设为可返佣状态", "result"=>1));
			exit();
		}catch(Exception $e){
		...
		try {
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
				try {
					...
					$message = array('msg'=>"客户订单款项添加成功", 'result'=>1, "url"=>spUrl("clientoverseas", "clientorderfundlist", array("client_id"=>$client_id)));
			...
			$this->client_id = $client_id;
			$this->saveurl = spUrl("clientoverseas", "createorderfund");
			$this->display("clientsales/createorderfund.html");
		}catch(Exception $e){
			$this->redirect(spUrl("clientoverseas", "clientorderfundlist", array("client_id"=>$client_id)), $e->getMessage());
		}
	}
